...
async def list_available_azure_openai_models(url: str, api_key: str, api_version:str) -> list[str]:
    client = AsyncAzureOpenAI(api_key=api_key, base_url=url, api_version=api_version)
    models = (await client.models.list()).data
    if models:
        return list(map(lambda x: x.id, models))
    return []
